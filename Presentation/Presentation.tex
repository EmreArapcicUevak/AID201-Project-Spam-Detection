...
\begin{document}
    ...
    \begin{frame}{How does the dataset look?}
        \input{../Tables/Spam_Before_Processing.tex}
    \end{frame}
    
    \begin{frame}{Visualization of missing data}
        \centering
        \includegraphics[height = 0.96\textheight, clip={0 0 0 0}]{../Images/Missing_Values_Pre_Processing_Visualized.pdf}
    \end{frame}

    \begin{frame}{Taking a closer look at the unnamed columns}
        \only<1>{
            \includegraphics[height = 0.85\textheight]{../Images/First_Character_Count.pdf}
        }

        \only<2>{
            \input{../Tables/Mistake_Values_Example_Table.tex}
        }
    \end{frame}
    
    \begin{frame}{After fixing up all of the mistakes}
        \input{../Tables/Spam_After_Processing.tex}
    \end{frame}

    \begin{frame}{Distribution of target class}
        \centering
        \only<1> {
            \includegraphics[height = 0.85\textheight]{../Images/Spam_Count.pdf}
        }

        \only<2> {
            \includegraphics[height = 0.85\textheight]{../Images/Spam_Class_Distribution.pdf}
        }
    \end{frame}
    \section{Model}
